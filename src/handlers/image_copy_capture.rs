...
pub fn output_capture_constraints(
    ...
    let dma = (|| {
        // From the renderer (instead of the backend) so it works under winit.
        let egl = renderer.egl_context();
        let node = EGLDevice::device_for_display(egl.display())
            .ok()?
            .try_get_render_node()
            .ok()??;
...
impl ImageCopyCaptureHandler for State {
    ...
    fn capture_constraints(&mut self, source: &ImageCaptureSource) -> Option<BufferConstraints> {
        ...
        self.backend
            .with_primary_renderer(|renderer| output_capture_constraints(renderer, &output))
            .flatten()
    }
